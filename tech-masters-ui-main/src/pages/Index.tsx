 ...
 * Index.tsx – Home Page
 * ──────────────────────
 * Performance strategy (Amazon-style):
 *
 *  1. Hero section, highlights & nav render INSTANTLY (zero API wait).
 *  2. A SINGLE API call (/api/homepage) fetches all section data at once.
 *  3. While data loads, sections show attractive skeleton placeholders
 *     instead of a full-screen spinner – the page feels fast and alive.
 *  4. Once the bundle arrives, everything pops in smoothly.
 */
...
import { useHomepageBundle }   from '@/hooks/useHomepageQuery';
import { mapApiProductToCard } from '@/lib/productMapping';

// ── Static data (never needs a network request) ───────────────────────────────

const highlights = [
  { icon: Truck,      title: '50-Hour Delivery',    description: 'Lightning-fast dispatch from our local warehouse' },
  { icon: Shield,     title: 'Trusted Components',  description: 'Sourced from verified manufacturers only' },
  { icon: Zap,        title: 'Local Fast Dispatch',  description: 'Same-day processing for quick turnaround' },
  { icon: BadgeCheck, title: 'Quality Tested',       description: 'Every component undergoes rigorous QC checks' },
];
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card overflow-hidden animate-pulse">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="h-5 bg-muted/60 rounded w-1/3 mt-2" />
      </div>
    </div>
  );
}
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="rounded-xl border border-border/30 p-4 animate-pulse">
          <div className="h-12 w-12 mx-auto rounded-lg bg-muted/60 mb-3" />
          <div className="h-3 bg-muted/60 rounded w-3/4 mx-auto" />
          <div className="h-2 bg-muted/40 rounded w-1/2 mx-auto mt-1" />
        </div>
      ))}
    </div>
  );
...
const Index = () => {
  ...
  const { data: bundle, isPending } = useHomepageBundle();

  // Map API data → typed card objects (memoised so we don't re-map on re-render)
  const categories = useMemo(
    () =>
      (bundle?.categories ?? []).slice(0, 8).map((c, index) => ({
        id:           index,
        name:         c.name,
        slug:         c.slug ?? c.name,
  ...
  const featuredProducts = useMemo(
    () => (bundle?.featured?.items ?? []).map((p) => mapApiProductToCard(p as Record<string, unknown>)),
    [bundle?.featured]
  );
  const popularProducts = useMemo(
    () => (bundle?.popular?.items ?? []).map((p) => mapApiProductToCard(p as Record<string, unknown>)),
    [bundle?.popular]
  );
  const newArrivals = useMemo(
    () => (bundle?.newest?.items ?? []).map((p) => mapApiProductToCard(p as Record<string, unknown>)),
    [bundle?.newest]
  );

  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* ── Navbar renders IMMEDIATELY (no API dependency) ─────────────── */}
      <Navbar />
      ...
      {/* ── Highlights – renders IMMEDIATELY ───────────────────────────── */}
      ...
      {/* ── Categories – skeleton while loading ────────────────────────── */}
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl md:text-3xl font-bold">Shop by Category</h2>
            <Button variant="ghost" asChild>
              <Link to="/products" className="gap-1">View All <ChevronRight className="h-4 w-4" /></Link>
            </Button>
          </div>
          ...
          {isPending ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No categories found. Start uploading products!
            </div>
          )}
        </div>
      </section>

      {/* ── Featured Products – skeleton while loading ──────────────────── */}
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            <div>
          ...
          {isPending ? (
            ...
            <p className="text-center py-8 text-muted-foreground">No featured products in this price range yet.</p>
          )}
        </div>
      </section>

      {/* ── Popular Components – skeleton while loading ─────────────────── */}
      ...
      {/* ── CTA Banner – renders IMMEDIATELY ───────────────────────────── */}
